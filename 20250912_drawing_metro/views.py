...
#        group1.grid(row = 1, column = 0, padx = 5, pady = 5)
        self.y_label = ["y轴", "速度(km/h)", "时间(s)", "网侧电流(A)", "电机电流(A)", "阻力(kN)", "累计能耗(kWh)", "电机输出功率(kW)", "网侧输入功率(kW)", "加速度(m/s^2)"]
        self.colors = ['red', 'green', 'blue', 'yellow', 'cyan', 'magenta', 'black', 'chocolate', 'orange']
        #  创建默认y轴范围
        self.yRange_default = ['[0, 100]', '[0, 300]', '[-5000, 5000]', '[-500, 500]', '[-500, 500]', '[0, 500]', '[-800, 800]', '[-8000, 8000]', '[-5, 5]']
        #  创建默认刻度格数
        self.scale_default = [10, 10, 10, 10, 10, 10, 10, 10, 10]
...
#        self.stop_event = threading.Event()
    ...
    def create_page(self):
        ...
        ttk.Label(self.group1, text="明细数据命名格式示例：明细_1500V_4M2T_AW3_上行").grid(row=0, column=0, padx=5, pady=5, columnspan=3)
...
#        sc.grid(row=0, column=0)
        ...
        add_button1.grid(row=10, column=2, padx=10, pady=10)
...
#        run_button.pack()
        ...
        ttk.Label(self.group5, text="额定网压(例如:1500V)").grid(row=0, column=0, padx=10, pady=10)
