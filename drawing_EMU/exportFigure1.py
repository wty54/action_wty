...
#    plt.figure(figsize=(21, 9))  #20251219设置图幅为16:9宽屏
    axs = {}  #创建字典用于存放y轴
    #    lines = []
#    labels = []
...
#    check_xls(Load_data, modify_data)
    ...
    Energy = ListWithAttribute(df1['累计能耗(包括牵引加辅助)(kWh)'].tolist())
    Accelerate = ListWithAttribute(df1['加速度(m/s^2)'].tolist())
    Kilo2 = ListWithAttribute(df1['公里标(km)'].tolist())  #按停站计算距离
...
#                    print(Period2[x0])
#                    Period2_temp = Period2[x0]
                    Kilo2[x] = abs(Kilo2[x] - Kilo2_temp)
